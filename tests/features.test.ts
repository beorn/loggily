...
describe("withRedaction", () => {
  ...
  test("protects every sink used through the default createLogger factory", () => {
    ...
    log.error?.("request failed with sk-message-secret", {
      authorization: "Bearer structured-secret",
    })

    expect(capture.lines).toHaveLength(1)
    expect(capture.lines[0]).not.toMatch(/message-secret|structured-secret/u)
    expect(capture.lines[0]).toContain(replacement)
  })
  ...
  test("covers known credential forms without misclassifying opaque identifiers", () => {
    ...
    const awsAccessKeyId = "AKIAIOSFODNN7EXAMPLE" // secret-scan-allow: synthetic fixture
