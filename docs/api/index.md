...
| Export                            | Description                                                                               |
| --------------------------------- | ----------------------------------------------------------------------------------------- |
| `createLogger(name, config?)`     | Create a conditional logger (includes redaction, env defaults, spans, and config metrics) |
...
`withRedaction()` is included by default in `createLogger()` and is browser-safe. Custom factories built from `baseCreateLogger()` opt in explicitly. The plugin returns new events, preserves correlation fields, tolerates circular structured data, and redacts messages, deep props, raw arguments, errors, and span props. Pass `{ replacement: "<hidden>" }` to change the default `[REDACTED]` marker.
