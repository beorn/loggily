...
`withRedaction()` is opt-in. Keep it before `withEnvDefaults()` as shown: the environment plugin forwards events to console, file, and global writers, so redaction must run first. The plugin replaces common credential keys and bearer, `sk-…`, or long token-shaped values across messages, structured props, raw arguments, errors, and spans.
